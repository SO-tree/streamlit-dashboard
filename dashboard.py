...
with tab3:
    ...
    # ✅ 평균 세션 길이 (수정: Matplotlib로 히스토그램 출력)
...
with tab4:
    ...
    fig, ax = plt.subplots(figsize=(5, 5))
    ax.pie([df['유저 ID'].nunique() - df[df['결제 금액'] > 0]['유저 ID'].nunique(), 
            df[df['결제 금액'] > 0]['유저 ID'].nunique()], 
           labels=["무료 유저", "유료 유저"], autopct='%1.1f%%', startangle=90, colors=["lightgray", "gold"])
